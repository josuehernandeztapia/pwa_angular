...
@Component({
  ...
  template: `
    <div class="event-log rounded-xl border p-6" style="background: var(--bg-dark); border-color: var(--border-dark);">
      <!-- Header -->
      <div class="log-header mb-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h3 class="text-xl font-bold" style="color: var(--text-light)">Historial de Eventos</h3>
              <p class="text-sm" style="color: var(--text-2)">Registro cronológico de actividades</p>
            </div>
          </div>
          <div class="log-controls flex items-center gap-3">
            ...
            <div class="filter-buttons flex items-center gap-2">
              <button 
                ...
                [class]="eventFilter === 'all' ? 'bg-primary-cyan-600 text-white' : 'bg-[var(--surface-dark)] text-[var(--text-2)] hover:bg-[var(--border-dark)]'"
              >
                Todos
              </button>
              <button 
                ...
                class="filter-btn px-3 py-1 text-xs font-medium rounded-full transition-colors"
                [class]="eventFilter === 'payments' ? 'bg-emerald-600 text-white' : 'bg-[var(--surface-dark)] text-[var(--text-2)] hover:bg-[var(--border-dark)]'"
              >
                Pagos
              </button>
              <button 
                ...
                class="filter-btn px-3 py-1 text-xs font-medium rounded-full transition-colors"
                [class]="eventFilter === 'documents' ? 'bg-amber-600 text-white' : 'bg-[var(--surface-dark)] text-[var(--text-2)] hover:bg-[var(--border-dark)]'"
              >
                Documentos
              </button>
              <button 
                ...
                class="filter-btn px-3 py-1 text-xs font-medium rounded-full transition-colors"
                [class]="eventFilter === 'system' ? 'bg-blue-600 text-white' : 'bg-[var(--surface-dark)] text-[var(--text-2)] hover:bg-[var(--border-dark)]'"
              >
                Sistema
              </button>
            </div>
            ...
            <!-- View Mode Toggle -->
            <div class="view-toggle flex items-center rounded-lg p-1" style="background: var(--surface-dark);">
              <button 
                (click)="setViewMode('timeline')"
                class="toggle-btn px-3 py-1 text-xs font-medium rounded-md transition-colors"
                [class]="viewMode === 'timeline' ? 'bg-primary-cyan-600 text-white' : 'text-[var(--text-2)] hover:text-[var(--text-light)]'"
              >
                Timeline
              </button>
              <button 
                ...
                class="toggle-btn px-3 py-1 text-xs font-medium rounded-md transition-colors"
                [class]="viewMode === 'list' ? 'bg-primary-cyan-600 text-white' : 'text-[var(--text-2)] hover:text-[var(--text-light)]'"
              >
                Lista
              </button>
            </div>
      ...
      <div class="timeline-view" *ngIf="viewMode === 'timeline' && filteredEvents.length > 0">
        <div class="timeline-container relative">
          <!-- Timeline Line -->
          <div class="timeline-line absolute left-8 top-0 bottom-0 w-0.5" style="background: var(--border-dark);"></div>
          ...
          <div class="event-groups space-y-8">
            <div *ngFor="let group of getEventGroups(); trackBy: trackByDate" class="event-group">
              <!-- Date Header -->
              <div class="date-header sticky top-0 z-10 py-3 mb-4" style="background: rgba(2,6,23,0.8)">
                <div class="flex items-center gap-4">
                  ...
                  <div class="date-line flex-1 h-px" style="background: var(--border-dark)"></div>
                  <span class="event-count px-3 py-1 rounded-full text-sm" style="background: var(--surface-dark); color: var(--text-2)">
                    ...
                    <div class="event-header flex items-center justify-between mb-3">
                      <div class="flex items-center gap-3">
                        ...
                        <div class="event-meta">
                          <h5 class="event-title font-semibold text-sm" style="color: var(--text-light)">{{ event.message }}</h5>
                          <div class="event-details flex items-center gap-2 text-xs mt-1" style="color: var(--text-2)">
                    ...
                    <div class="event-actions mt-3 flex items-center gap-2" *ngIf="hasEventActions(event)">
                      <button 
                        *ngIf="canViewDetails(event)"
                        ...
                        class="action-btn px-3 py-1 rounded-md text-xs transition-colors bg-[var(--surface-dark)] text-[var(--text-2)] hover:bg-[var(--border-dark)] hover:text-[var(--text-light)]"
                      >
                        Ver Detalles
                      </button>
      ...
      <div class="list-view" *ngIf="viewMode === 'list' && filteredEvents.length > 0">
        <div class="events-table">
          <div class="table-header grid grid-cols-12 gap-4 p-3 rounded-t-lg text-sm font-medium" style="background: var(--surface-dark); color: var(--text-2)">
          ...
          <div class="table-body">
            <div 
              *ngFor="let event of filteredEvents; trackBy: trackByEventId" 
              class="table-row grid grid-cols-12 gap-4 p-3 border-b transition-colors" style="border-color: var(--border-dark)"
            >
              ...
              <div class="col-span-4 flex items-center">
                <span class="text-sm" style="color: var(--text-light)">{{ event.message }}</span>
              </div>
              ...
              <div class="col-span-2 flex items-center">
                ...
                <span *ngIf="!event.details?.amount" class="text-sm" style="color: var(--text-2)">-</span>
              </div>
              
              <div class="col-span-2 flex items-center">
                <span class="text-sm" style="color: var(--text-2)">{{ formatDateTime(event.timestamp) }}</span>
              </div>
              ...
              <div class="col-span-1 flex items-center">
                <button 
                  *ngIf="hasEventActions(event)"
                  ...
                  class="action-menu-btn p-1 rounded-md transition-colors hover:bg-[var(--surface-dark)]"
                >
                  <span class="hover:text-[var(--text-light)]" style="color: var(--text-2)">⋮</span>
                </button>
              </div>
            </div>
          </div>
      ...
      <div class="empty-state text-center py-12" *ngIf="filteredEvents.length === 0">
        ...
        <h4 class="text-xl font-semibold mb-2" style="color: var(--text-2)">No hay eventos</h4>
        <p style="color: var(--text-2)">{{ getEmptyStateMessage() }}</p>
      </div>
      ...
      <div class="load-more text-center mt-6" *ngIf="hasMoreEvents">
        <button 
          ...
          class="load-more-btn px-6 py-3 rounded-lg font-medium transition-colors disabled:opacity-50 text-[var(--text-light)] bg-[var(--surface-dark)] hover:bg-[var(--border-dark)]"
        >
