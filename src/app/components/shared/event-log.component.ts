    ...
    <div class="event-log rounded-xl border p-6" style="background: var(--bg-dark); border-color: var(--border-dark);">
          ...
          <div class="log-controls flex items-center gap-3">
            ...
            <div class="view-toggle flex items-center rounded-lg p-1" style="background: var(--surface-dark);">
              <button 
                ...
                [class]="viewMode === 'timeline' ? 'bg-primary-cyan-600 text-white' : 'text-[var(--text-2)] hover:text-white'"
              >
                Timeline
              </button>
              <button 
                ...
                class="toggle-btn px-3 py-1 text-xs font-medium rounded-md transition-colors"
                [class]="viewMode === 'list' ? 'bg-primary-cyan-600 text-white' : 'text-[var(--text-2)] hover:text-white'"
              >
                Lista
              </button>
            </div>
        ...
        <div class="timeline-container relative">
          ...
          <div class="event-groups space-y-8">
            <div *ngFor="let group of getEventGroups(); trackBy: trackByDate" class="event-group">
              <!-- Date Header -->
              <div class="date-header sticky top-0 z-10 py-3 mb-4" style="background: var(--bg-dark);">
                <div class="flex items-center gap-4">
                  <div class="date-marker w-4 h-4 bg-primary-cyan-500 rounded-full relative z-10 flex-shrink-0 ml-6"></div>
                  <h4 class="text-lg font-semibold" style="color: var(--text-light)">{{ group.date }}</h4>
                    ...
                    <div *ngIf="hasEventDetails(event)" class="event-details-section">
                      <div class="details-grid grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div *ngIf="event.details?.paymentMethod" class="detail-item">
                          <span class="detail-label text-xs" style="color: var(--text-2)">Método de Pago:</span>
                        ...
                        <div *ngIf="event.details?.reference" class="detail-item">
                          <span class="detail-label text-xs" style="color: var(--text-2)">Referencia:</span>
                        ...
                        <div *ngIf="event.details?.documentName" class="detail-item">
                          <span class="detail-label text-xs" style="color: var(--text-2)">Documento:</span>
                        ...
                        <div *ngIf="event.details?.status" class="detail-item">
                          <span class="detail-label text-xs" style="color: var(--text-2)">Estado:</span>
                    ...
                    <div class="event-actions mt-3 flex items-center gap-2" *ngIf="hasEventActions(event)">
                      <button 
                        *ngIf="canViewDetails(event)"
                        ...
                        class="action-btn px-3 py-1 rounded-md text-xs transition-colors"
                        style="background: var(--surface-dark); color: var(--text-2)"
                      >
                        Ver Detalles
                      </button>
          ...
          <div class="table-body">
            <div 
              *ngFor="let event of filteredEvents; trackBy: trackByEventId" 
              class="table-row grid grid-cols-12 gap-4 p-3 border-b transition-colors"
              ...
              <div class="col-span-1 flex items-center">
                <button 
                  *ngIf="hasEventActions(event)"
                  ...
                  class="action-menu-btn p-1 rounded-md transition-colors"
                >
      ...
      <div class="load-more text-center mt-6" *ngIf="hasMoreEvents">
        <button 
          ...
          class="load-more-btn px-6 py-3 text-white rounded-lg font-medium transition-colors disabled:opacity-50"
