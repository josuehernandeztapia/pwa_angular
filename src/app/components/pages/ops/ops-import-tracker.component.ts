    ...
    <div class="p-6 space-y-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600">Estado de importación, entregas y triggers en una vista</p>
        </div>
...
export class OpsImportTrackerComponent implements OnDestroy {
  ...
  constructor(private tracker: IntegratedImportTrackerService) {
    ...
    // Auto-fetch on clientId change (debounced)
  ...
  setMilestone(milestone: keyof IntegratedImportStatus, state: 'completed' | 'in_progress' | 'pending') {
    const id = (this.clientIdCtrl.value || '').trim();
    if (!id) return;
